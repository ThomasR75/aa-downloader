    ...
    with open(output_csv_path, 'a', newline='', encoding='utf-8') as out_f:
        ...
        for book in books:
            ...
            try:
                ...
                # Deduplicate while preserving order, try up to 3 results
                seen = set()
                unique_md5s = []
                for m in md5_matches:
                    if m not in seen:
                        seen.add(m)
                        unique_md5s.append(m)
                candidates = unique_md5s[:3]

                book_downloaded = False

                for attempt, target_md5_path in enumerate(candidates, start=1):
                    book_page_url = base_url.rstrip('/') + target_md5_path
                    print(f"Trying result {attempt}/{len(candidates)}: {book_page_url}")

                    time.sleep(2)
                    book_page_res = requests.get(book_page_url, headers=headers, timeout=15)
                    if book_page_res.status_code != 200:
                        print(f"Could not access book page (Status {book_page_res.status_code}), trying next result.")
                        continue
                    ...
                    if not found_download_url and fast_download_links_absolute:
                        ...
                        print(f"No explicit EPUB/MOBI fast download link, trying first fast download: {found_download_url}")
                        ...
                        if '.mobi' in found_download_url.lower() or 'format=mobi' in found_download_url.lower():
                            ...
                            expected_extension = '.pdf'
                        print(f"No fast download link, trying first available: {found_download_url}")

                    if not found_download_url:
                        print(f"No viable download link on result {attempt}, trying next result.")
                        continue
                    ...
                    try:
                        ...
                        if file_is_valid:
                            ...
                            print(f"Verification failed for result {attempt} ({filename}), trying next result.")
                            if filepath and os.path.exists(filepath):
                                os.remove(filepath)

                    except requests.exceptions.HTTPError as http_err:
                        print(f"HTTP error on result {attempt} ({http_err.response.status_code}), trying next result.")
                        if filepath and os.path.exists(filepath):
                            os.remove(filepath)
                    except requests.exceptions.RequestException as req_err:
                        print(f"Request error on result {attempt}: {req_err}, trying next result.")
                        if filepath and os.path.exists(filepath):
                            os.remove(filepath)
                    except Exception as e:
                        print(f"Unexpected error on result {attempt}: {e}, trying next result.")
                        if filepath and os.path.exists(filepath):
                            os.remove(filepath)

                if not book_downloaded:
                    print(f"All {len(candidates)} result(s) failed for '{title}' — skipping.")
